...
class Spree::PaymentMethod::WirecardCheckoutPageInvoice < Spree::PaymentMethod::WirecardCheckoutPage

  preference :min_amount, :decimal
  preference :max_amount, :decimal

  #attr_accessible :preferred_min_amount, :preferred_max_amount
  self.paymenttype = 'INVOICE'
  ...
  def allowed?(order, urls, extra_params, request)
    raise I18n::t('wirecard_checkout_page.birthday_required') if extra_params[:birthday].nil?
    raise I18n::t('wirecard_checkout_page.invoice_notallowed') unless visible(:front_end, order)
    raise I18n::t('wirecard_checkout_page.invoice_min_age') if calculate_age(extra_params[:birthday]) < self.min_age
  end
